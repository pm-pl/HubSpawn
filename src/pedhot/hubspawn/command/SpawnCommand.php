...
class SpawnCommand extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void {
        ...
        if (!$sender->hasPermission("hubspawn.spawn")) {
            $sender->sendMessage(Server::getInstance()->getLanguage()->translateString(TextFormat::RED . "%commands.generic.permission"));
            return;
        }

        $sender->sendMessage(HubSpawn::formatter($sender, HubSpawn::getInstance()->getMessage()->getNested(HubSpawn::getInstance()->getSelectedLang().".spawn-teleport")));
        $sender->teleport(HubSpawn::teleportTo("spawn"));
    }

    protected function prepare(): void {
        $this->setAliases(HubSpawn::getInstance()->getConfig()->getNested("commands-settings.spawn.aliases"));
        $this->setPermission("hubspawn.spawn");
        $this->setDescription(HubSpawn::getInstance()->getConfig()->getNested("commands-settings.spawn.description"));
